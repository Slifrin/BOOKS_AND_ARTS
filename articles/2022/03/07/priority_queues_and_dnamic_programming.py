    ...
    for dx in [-1, 0, 1]:
        for dy in [-1, 0, 1]:
            if dx == 0 and dy == 0:
                continue
            possition = x + dx, y + dy
            if is_valid(lines, possition):
                yield possition

def get_shorter_paths(tenative, positions, through):
    path = tenative[through] + [through]
    for position in positions:
        if position in tenative and len(tenative[position]) <= len(path):
            continue
        yield position, path
...
def find_path(map):
    ...
    while destination not in certain and len(candidates) > 0:
        ...
        shorter = get_shorter_paths(tenative, neighbors, current)
        for neighbor, path in shorter:
            tenative[neighbor] = path
            heapq.heappush(candidates, (len(path), neighbor))
    if destination in tenative:
        return tenative[destination] + [destination]
    else:
