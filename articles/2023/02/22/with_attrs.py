...
def percentage_value(
    instance: type, attribute: Attribute[str], value: float
) -> None:
    class_name = instance.__class__.__name__
    if not 0 <= value <= 1:
        raise ValueError(
...
"""
Possible to add validator as decorator
...
class Customer:
    ...
    def _check_percentage(slef, attribute, value):
        if not 0 <= value <= 1:
            raise ValueError("Tarapaty)
"""
@define
class Product:
